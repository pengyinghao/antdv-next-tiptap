...
| TypeScript    | 暴露 `EditorProps`、`EditorPlugin`、`PluginName`、`OutputFormat` 等类型    |
...
```vue
<template>
  <AEditor v-model="content" :height="420" />
</template>
...
<script setup lang="ts">
import { ref } from 'vue'
import { AEditor } from 'antdv-next-tiptap'
import 'antdv-next-tiptap/index.css'
...
```vue
<AEditor :disabled-plugins="['image', 'video', 'table']" />
```
...
```vue
<AEditor v-model="content" output-format="json" />
```

### 只读模式

```vue
<AEditor v-model="content" :editable="false" />
```
...
```vue
<template>
  <AEditor v-model="content" :upload-video="uploadVideo" />
</template>
